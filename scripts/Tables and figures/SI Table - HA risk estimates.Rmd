---
title: "SI Table - HA risk estimates"
author: "Ruwan T"
date: "6/10/2021"
output:
  html_document:
...
##  knit to HTML, open in browser, copy table to Excel, format in Excel then copy to Word; then added caption and footnotes in Word to produce:
# S:\Wildfire-Hoshiko\Wildfire - RThilakaratne\PHIRE_Health_Analysis\output\Paper 1 - Tables and Figures\Table - HA risk estimates\Table SX - HA risk estimates.docx
...
```{r,echo=FALSE}
results <- readRDS("S:/Wildfire-Hoshiko/Wildfire - RThilakaratne/PHIRE_Health_Analysis/output/Risk estimate models/pdd_all_results.rds")
...
Respiratory <- results2%>%filter(Outcome=="Respiratory* (Lag 0-1)")%>%dplyr::select(Subgroup,risk) %>% rename(Respiratory=risk)
Cardiovascular <- results2%>%filter(Outcome=="Cardiovascular** (Lag 0-1)")%>%dplyr::select(Subgroup,risk) %>% rename(Cardiovascular=risk)
Asthma <- results2%>%filter(Outcome=="Asthma (Lag 0-1)")%>%dplyr::select(Subgroup,risk) %>% rename(Asthma=risk)
...
names(table6) <- c("Subgroup","Respiratory* (Lag 0-1)","Asthma (Lag 0-1)","COPD (Lag 0-1)","Cardiovascular** (Lag 0-1)")
