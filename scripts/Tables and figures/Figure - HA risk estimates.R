...
results <- readRDS("output/Risk estimate models/pdd_all_results.rds")
# results$indicator <- 0
results$indicator <- ifelse(results$Subgroup=="NH Nat Am",1,0)

asthma_plot <- ggplot(data=results%>%filter(Outcome=="Asthma (Lag 0-1)",indicator==0)) +
  ylim(c(-0.1,4.5)) +
...
resp_plot <- ggplot(data=results%>%filter(Outcome=="Respiratory* (Lag 0-1)",indicator==0)) +
  geom_point(aes(x=Subgroup,y=RR)) +
  geom_errorbar(aes(x=Subgroup,ymin=LB,ymax=UB),width=0.3) +
  facet_wrap(~ Outcome, ncol=1,scales="free") +
  ylab("Change in risk (%)") +
...
cv_plot <- ggplot(data=results%>%filter(Outcome=="Cardiovascular** (Lag 0-1)",indicator==0)) +
  geom_point(aes(x=Subgroup,y=RR)) +
  geom_errorbar(aes(x=Subgroup,ymin=LB,ymax=UB),width=0.3) +
  facet_wrap(~ Outcome, ncol=1,scales="free") +
  ylab("Change in risk (%)") +
...
png(filename="output/Paper 1 - Tables and Figures/Figure - HA risk estimates/Figure - HA risk estimates.png",units="in",width=11,height=8.5,res=1200)
