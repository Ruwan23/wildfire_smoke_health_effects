# "PHIRE Data Analysis - All-cause asthma ED model building Part 1"
# "Ruwan T"
# "5/11/2021"

# notes: 
# - different lag codings
# - only asthma ED shows piece-by-piece building
# - R markdown frequently crashed due to size of models
# - for each outcome, DLM comparisons and lag plot are saved
# - tried to pick time spline by BIC but fit kept improving so opted for a priori selection of 4df/yr
...
#------------------------
data_harv$zip3 <- substr(data_harv$PATZIP,1,3)
locations <- c("San Francisco","Sacramento","Redding","Los Angeles","Fresno")
locations_zip3 <-  c(940,958,960,900,937)
data4_ts <- data_harv %>% filter(zip3 %in% locations_zip3)
data4_ts$serv_dt <- as.Date(data4_ts$serv_dt)
for (i in 1:length(locations_zip3)){
  df <- data4_ts %>% filter(zip3==locations_zip3[i])
  df2 <- df %>% group_by(serv_dt)%>%summarize(asthma_sum = sum(ed_asthma))
  y <- ggplot(data=df2) + geom_point(aes(x=serv_dt,y=asthma_sum)) + 
    xlab("Date") + ylab("Total visits") + ggtitle(paste(locations[i],"All-cause asthma ED (any dx) visits over time",sep=" ")) +
    scale_x_date(date_breaks = "1 year",date_labels = "%Y")
  print(y)
}


data_col <- data_harv %>% group_by(serv_dt)%>%summarize(asthma_sum = sum(ed_asthma))
dates_vline <- as.Date(c("2015-10-01")) 
dates_vline <- which(data_col$serv_dt %in% dates_vline)
ggplot(data=data_col) + geom_point(aes(x=serv_dt,y=asthma_sum)) + 
  xlab("Date") + ylab("Total visits") + ggtitle(paste("California","All-cause asthma ED (any dx) visits over time",sep=" ")) +
  scale_x_date(date_breaks = "1 year",date_labels = "%Y") +
...
# pm25 residuals
tic <- Sys.time()
model_1 <- speedglm(ed_asthma ~ pm25_residual_cz,data = data4,family=poisson(link="log"))
toc <- Sys.time()
model_1_results <- c("asthma ~ pm25_residual_cz",
                     "exposure only",
...
# pm25 + zip code
tic <- Sys.time()
model_2 <- glmer(ed_asthma ~ pm25_residual_cz + (1|PATZIP),data = data4,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()
model_2_results <- c("asthma ~ pm25_residual_cz + (1|PATZIP)",
                     "+ zip code RI",
                     round(as.numeric(difftime(toc,tic),units="secs"),2),
...
tic <- Sys.time()
model_3 <- glmer(ed_asthma ~ pm25_residual_cz + ns(dayseq,36) + (1|PATZIP),data = data4,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()
model_3_results <- c("asthma ~ pm25_residual_cz + ns(dayseq,36) + (1|PATZIP)",
                     "+ time function",
                     round(as.numeric(difftime(toc,tic),units="secs"),2),
...
tic <- Sys.time()
model_4 <- glmer(ed_asthma ~ pm25_residual_cz + dow + ns(dayseq,36) + (1|PATZIP),data = data4,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()
model_4_results <- c("asthma ~ pm25_residual_cz + dow + ns(dayseq,36) + (1|PATZIP)",
                     "+ DOW",
                     round(as.numeric(difftime(toc,tic),units="secs"),2),
...
write.csv(results_dfselection,"output/Asthma ED/Model building/Time spline selection.csv")
...
tic <- Sys.time()
model_6 <- glmer(ed_asthma ~ pm25_residual_cz + ns(hi,3) + dow + ns(dayseq,36) + icd10 + (1|PATZIP),data = data4,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()
model_6_results <- c("asthma ~ pm25_residual_cz + ns(hi,3) + dow + ns(dayseq,36) + ICD-10 + (1|PATZIP)",
                     "+ ICD-10 introduction",
                     round(as.numeric(difftime(toc,tic),units="secs"),2),
...
# pm25 + DOW + heat index + spline(4/y) + ICD change + zip code RI
tic <- Sys.time()
model_lag0 <- glmer(ed_asthma ~ pm25_residual_cz + ns(hi,3) + ns(dayseq,36) + dow + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()
model_lag0_results <- c("asthma ~ pm25_residual_cz + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  ...
  ggtitle("DLM 0 for association between PM2.5 (10-unit increase) and asthma ED visits") +
  # ylim(0.99,1.01) +
...
model_lag1 <- glmer(ed_asthma ~ pm25_residual_cz + pm25_residual_cz_lag1 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag1_results <- c("asthma ~ pm25_residual_cz + pm25_residual_cz_lag1 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0-1",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  geom_point() +
  geom_errorbar(aes(ymin=LB,ymax=UB)) +
  ggtitle("DLM 0-1 for association between PM2.5 (10-unit increase) and asthma ED visits") +
  # ylim(0.99,1.01) +
  geom_hline(yintercept=1.0, linetype="dotted")
...
model_lag2 <- glmer(ed_asthma ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag2_results <- c("asthma ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0-2",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  geom_point() +
  geom_errorbar(aes(ymin=LB,ymax=UB)) +
  ggtitle("DLM 0-2 for association between PM2.5 (10-unit increase) and asthma ED visits") +
  # ylim(0.99,1.01) +
  geom_hline(yintercept=1.0, linetype="dotted")
...
model_lag3 <- glmer(ed_asthma ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag3_results <- c("asthma ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0-3",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  geom_point() +
  geom_errorbar(aes(ymin=LB,ymax=UB)) +
  ggtitle("DLM 0-3 for association between PM2.5 (10-unit increase) and asthma ED visits") +
  # ylim(0.99,1.01) +
  geom_hline(yintercept=1.0, linetype="dotted")
...
model_lag4 <- glmer(ed_asthma ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag4_results <- c("asthma ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0-4",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  geom_point() +
  geom_errorbar(aes(ymin=LB,ymax=UB)) +
  ggtitle("DLM 0-4 for association between PM2.5 (10-unit increase) and asthma ED visits") +
  # ylim(0.99,1.01) +
  geom_hline(yintercept=1.0, linetype="dotted")
...
model_lag5 <- glmer(ed_asthma ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + pm25_residual_cz_lag5 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag5_results <- c("asthma ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + pm25_residual_cz_lag5 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0-5",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  geom_point() +
  geom_errorbar(aes(ymin=LB,ymax=UB)) +
  ggtitle("DLM 0-5 for association between PM2.5 (10-unit increase) and asthma ED visits") +
  # ylim(0.99,1.01) +
  geom_hline(yintercept=1.0, linetype="dotted")
...
## DLM 0-6
model_lag6 <- glmer(ed_asthma ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + pm25_residual_cz_lag5 + pm25_residual_cz_lag6 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag6_results <- c("asthma ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + pm25_residual_cz_lag5 + pm25_residual_cz_lag6 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0-6",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  geom_point() +
  geom_errorbar(aes(ymin=LB,ymax=UB)) +
  ggtitle("DLM 0-6 for association between PM2.5 (10-unit increase) and asthma ED visits") +
  # ylim(0.99,1.01) +
  geom_hline(yintercept=1.0, linetype="dotted")
