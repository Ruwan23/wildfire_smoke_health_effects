...
results_allcause_resp <- data.frame(matrix(NA,nrow=0,ncol=9))
names(results_allcause_resp) <- c("Model","Model change","Time (s)","Std error","RR","95%CI LB","95%CI UB","AIC","BIC")
...
tic <- Sys.time()
model_lag0 <- glmer(pdd_allcause_resp ~ pm25_residual_cz + ns(hi,3) + ns(dayseq,36) + dow + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()
model_lag0_results_allcause_resp <- c("pdd_allcause_resp ~ pm25_residual_cz + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                                      "DLM 0",
...
results_allcause_resp <- rbind(results_allcause_resp,model_lag0_results_allcause_resp)
summary(model_lag0)
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  ...
  ggtitle("DLM 0 for association between PM2.5 (10-unit increase) and Resp PDD visits") +
  # ylim(0.99,1.01) +
...
model_lag1 <- glmer(pdd_allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag1_results_allcause_resp <- c("pdd_allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                                      "DLM 0-1",
                                      round(as.numeric(difftime(toc,tic),units="secs"),2),
...
results_allcause_resp <- rbind(results_allcause_resp,model_lag1_results_allcause_resp)
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  geom_point() +
  geom_errorbar(aes(ymin=LB,ymax=UB)) +
  ggtitle("DLM 0-1 for association between PM2.5 (10-unit increase) and Resp PDD visits") +
  # ylim(0.99,1.01) +
  geom_hline(yintercept=1.0, linetype="dotted")
...
model_lag2 <- glmer(pdd_allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag2_results_allcause_resp <- c("pdd_allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                                      "DLM 0-2",
                                      round(as.numeric(difftime(toc,tic),units="secs"),2),
...
results_allcause_resp <- rbind(results_allcause_resp,model_lag2_results_allcause_resp)
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  geom_point() +
  geom_errorbar(aes(ymin=LB,ymax=UB)) +
  ggtitle("DLM 0-2 for association between PM2.5 (10-unit increase) and Resp PDD visits") +
  # ylim(0.99,1.01) +
  geom_hline(yintercept=1.0, linetype="dotted")
...
model_lag3 <- glmer(pdd_allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag3_results_allcause_resp <- c("pdd_allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                                      "DLM 0-3",
                                      round(as.numeric(difftime(toc,tic),units="secs"),2),
...
results_allcause_resp <- rbind(results_allcause_resp,model_lag3_results_allcause_resp)
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  geom_point() +
  geom_errorbar(aes(ymin=LB,ymax=UB)) +
  ggtitle("DLM 0-3 for association between PM2.5 (10-unit increase) and Resp PDD visits") +
  # ylim(0.99,1.01) +
  geom_hline(yintercept=1.0, linetype="dotted")
...
model_lag4 <- glmer(pdd_allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag4_results_allcause_resp <- c("pdd_allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                                      "DLM 0-4",
                                      round(as.numeric(difftime(toc,tic),units="secs"),2),
...
results_allcause_resp <- rbind(results_allcause_resp,model_lag4_results_allcause_resp)
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  geom_point() +
  geom_errorbar(aes(ymin=LB,ymax=UB)) +
  ggtitle("DLM 0-4 for association between PM2.5 (10-unit increase) and Resp PDD visits") +
  # ylim(0.99,1.01) +
  geom_hline(yintercept=1.0, linetype="dotted")
...
model_lag5 <- glmer(pdd_allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + pm25_residual_cz_lag5 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag5_results_allcause_resp <- c("pdd_allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + pm25_residual_cz_lag5 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                                      "DLM 0-5",
                                      round(as.numeric(difftime(toc,tic),units="secs"),2),
...
results_allcause_resp <- rbind(results_allcause_resp,model_lag5_results_allcause_resp)
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  geom_point() +
  geom_errorbar(aes(ymin=LB,ymax=UB)) +
  ggtitle("DLM 0-5 for association between PM2.5 (10-unit increase) and Resp PDD visits") +
  # ylim(0.99,1.01) +
  geom_hline(yintercept=1.0, linetype="dotted")
...
## DLM 0-6
model_lag6 <- glmer(pdd_allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + pm25_residual_cz_lag5 + pm25_residual_cz_lag6 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag6_results_allcause_resp <- c("pdd_allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + pm25_residual_cz_lag5 + pm25_residual_cz_lag6 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                                      "DLM 0-6",
                                      round(as.numeric(difftime(toc,tic),units="secs"),2),
...
results_allcause_resp <- rbind(results_allcause_resp,model_lag6_results_allcause_resp)
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  geom_point() +
  geom_errorbar(aes(ymin=LB,ymax=UB)) +
  ggtitle("DLM 0-6 for association between PM2.5 (10-unit increase) and Resp PDD visits") +
  # ylim(0.99,1.01) +
  geom_hline(yintercept=1.0, linetype="dotted")


names(results_allcause_resp) <- c("Model","Model change","Time (s)","Std error","RR","95%CI LB","95%CI UB","AIC","BIC")
write.csv(results_allcause_resp,"output/Resp PDD/Resp PDD DLM models v2.csv")
