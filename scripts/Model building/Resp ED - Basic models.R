# "PHIRE Data Analysis - All-cause Resp ED model building Part 1"
# "Ruwan T"
# "5/11/2021"
...
for (i in 1:length(locations_zip3)){
  ...
  df2 <- df %>% group_by(serv_dt)%>%summarize(allcause_resp_sum = sum(allcause_resp))
  y <- ggplot(data=df2) + geom_point(aes(x=serv_dt,y=allcause_resp_sum)) + 
    xlab("Date") + ylab("Total visits") + ggtitle(paste(locations[i],"All-cause CV ED (any dx) visits over time",sep=" ")) +
    scale_x_date(date_breaks = "1 year",date_labels = "%Y")
  print(y)
}


data_col <- data4 %>% group_by(serv_dt)%>%summarize(allcause_resp_sum = sum(allcause_resp))
dates_vline <- as.Date(c("2015-10-01")) 
dates_vline <- which(data_col$serv_dt %in% dates_vline)
ggplot(data=data_col) + geom_point(aes(x=serv_dt,y=allcause_resp_sum)) + 
  xlab("Date") + ylab("Total visits") + ggtitle(paste("California","All-cause Resp ED (any dx) visits over time",sep=" ")) +
  scale_x_date(date_breaks = "1 year",date_labels = "%Y") +
...
# pm25 residuals
tic <- Sys.time()
model_1 <- speedglm(allcause_resp ~ pm25_residual_cz,data = data4,family=poisson(link="log"))
toc <- Sys.time()
model_1_results <- c("allcause_resp ~ pm25_residual_cz",
                     "exposure only",
...
# pm25 + zip code
tic <- Sys.time()
model_2 <- glmer(allcause_resp ~ pm25_residual_cz + (1|PATZIP),data = data4,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()
model_2_results <- c("allcause_resp ~ pm25_residual_cz + (1|PATZIP)",
                     "+ zip code RI",
                     round(as.numeric(difftime(toc,tic),units="secs"),2),
...
tic <- Sys.time()
model_3 <- glmer(allcause_resp ~ pm25_residual_cz + ns(dayseq,36) + (1|PATZIP),data = data4,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()
model_3_results <- c("allcause_resp ~ pm25_residual_cz + ns(dayseq,36) + (1|PATZIP)",
                     "+ time function",
                     round(as.numeric(difftime(toc,tic),units="secs"),2),
...
tic <- Sys.time()
model_4 <- glmer(allcause_resp ~ pm25_residual_cz + dow + ns(dayseq,36) + (1|PATZIP),data = data4,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()
model_4_results <- c("allcause_resp ~ pm25_residual_cz + dow + ns(dayseq,36) + (1|PATZIP)",
                     "+ DOW",
                     round(as.numeric(difftime(toc,tic),units="secs"),2),
...
tic <- Sys.time()
model_5 <- glmer(allcause_resp ~ pm25_residual_cz + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data4,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()
model_5_results <- c("allcause_resp ~ pm25 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                     "+ heat index",
                     round(as.numeric(difftime(toc,tic),units="secs"),2),
...
write.csv(results_dfselection,"output/Resp ED/Time spline selection.csv")
...
tic <- Sys.time()
model_6 <- glmer(allcause_resp ~ pm25_residual_cz + ns(hi,3) + dow + ns(dayseq,36) + icd10 + (1|PATZIP),data = data4,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()
model_6_results <- c("allcause_resp ~ pm25_residual_cz + ns(hi,3) + dow + ns(dayseq,36) + ICD-10 + (1|PATZIP)",
                     "+ ICD-10 introduction",
                     round(as.numeric(difftime(toc,tic),units="secs"),2),
...
# pm25 + DOW + heat index + spline(4/y) + ICD change + zip code RI
tic <- Sys.time()
model_lag0 <- glmer(allcause_resp ~ pm25_residual_cz + ns(hi,3) + ns(dayseq,36) + dow + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()
model_lag0_results <- c("allcause_resp ~ pm25_residual_cz + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
model_lag1 <- glmer(allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag1_results <- c("allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0-1",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
model_lag2 <- glmer(allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag2_results <- c("allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0-2",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
model_lag3 <- glmer(allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag3_results <- c("allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0-3",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
model_lag4 <- glmer(allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag4_results <- c("allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0-4",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
model_lag5 <- glmer(allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + pm25_residual_cz_lag5 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag5_results <- c("allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + pm25_residual_cz_lag5 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0-5",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
## DLM 0-6
model_lag6 <- glmer(allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + pm25_residual_cz_lag5 + pm25_residual_cz_lag6 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP),data = data6,family=poisson(link="log"),nAGQ = 0)
toc <- Sys.time()

model_lag6_results <- c("allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + pm25_residual_cz_lag5 + pm25_residual_cz_lag6 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0-6",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
## DLM 0-14
model_lag14 <- glmer(allcause_resp ~ pm25_residual_cz + pm25_residual_cz_lag1 + pm25_residual_cz_lag2 + pm25_residual_cz_lag3 + pm25_residual_cz_lag4 + pm25_residual_cz_lag5 + pm25_residual_cz_lag6 +
...
model_lag14_results <- c("allcause_resp_count ~ pm25_residual_cz + pm25_residual_cz_lag1-14 + ns(hi,3) + dow + ns(dayseq,36) + (1|PATZIP)",
                        "DLM 0-14",
                        round(as.numeric(difftime(toc,tic),units="secs"),2),
...
ggplot(data=toplot,aes(x=Lag,y=RR)) +
  geom_point() +
  geom_errorbar(aes(ymin=LB,ymax=UB)) +
  ggtitle("DLM 0-14 for association between PM2.5 (10-unit increase) and Resp ED visits") +
  # ylim(0.99,1.01) +
  geom_hline(yintercept=1.0, linetype="dotted")
